class TableManager {
    ...
    async loadData() {
        ...
        try {
            // Versiyonlu fetch - cache busting için version.txt kullanılır
            let dataUrl = 'data.json';
            try {
                const versionResp = await fetch('version.txt?t=' + Date.now());
                if (versionResp.ok) {
                    const version = await versionResp.text();
                    dataUrl = 'data.json?v=' + version.trim();
                }
            } catch (e) {
                // version.txt yoksa normal fetch yap
                console.log('version.txt bulunamadı, cache-busting atlanıyor');
            }
