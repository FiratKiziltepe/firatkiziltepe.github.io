...
${o.requireEvidence ? 'For every "yes" and every "no" verdict, copy a SHORT VERBATIM quote (max 25 words) from the record text as "evidence". Never paraphrase inside "evidence". Use "" for "unclear".\n' : ''}
...
{"results": [
  {
    ...
    "summary": "<1–2 sentence screening-relevant summary in ${o.summaryLanguage}>",
    "criteria_assessment": [
...
`;
  ...
  function buildResponseSchema(opts = {}) {
    const item = {
      ...
      propertyOrdering: ['id', 'summary', 'criteria_assessment', 'decision', 'confidence', 'needs_human_review', 'rationale']
    };
    if (opts.userTopic) {
  ...
  function validateModelRecord(raw, ctx) {
    ...
    if (flags.length) rationale += (rationale ? ' ' : '') + `[⚠️ Sistem denetimi: ${flags.join('; ')}]`;
  ...
  function consensus(modelResults, modelOrder, opts = {}) {
    ...
    if (unique.length > 1) {
      rationale = `[Model uyuşmazlığı: ${valid.map(m => `${m}=${modelResults[m].decision}`).join(', ')}] ` + rationale;
    }
    if (errored.length) rationale += ` [Hata veren model(ler): ${errored.join(', ')}]`;
  ...
  function prepareRecords(rows, opts = {}) {
    ...
    body.forEach((v, i) => {
      ...
      if (opts.dedupe !== false) {
        ...
        if (dup) { r.duplicateOf = dup; duplicates++; }
        else {
  ...
  function presetResult(record, decision, reason) {
    return {
      ...
      year: record.Year, doi: record.DOI, abstract: record.Abstract,
      ...
      rationale: reason, relevance_score: null, relevance_rationale: '', modelDecisions: {}, error: null,
      preset: true
    };
  }
  ...
  function finalizeRecord(record, modelResults, modelOrder, opts) {
    ...
    return {
      ...
      summary: c.summary, rationale: c.rationale,
